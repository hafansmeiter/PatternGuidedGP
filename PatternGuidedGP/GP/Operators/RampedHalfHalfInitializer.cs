...
namespace PatternGuidedGP.GP.Operators {
	class RampedHalfHalfInitializer : InitializerBase {
		...
		public override void Initialize(Population population, int maxTreeDepth) {
			_generatorFull.TreeNodeRepository = TreeNodeRepository;
			_generatorGrow.TreeNodeRepository = TreeNodeRepository;
			for (int i = 0; i < population.Size; i++) {
				SyntaxNode syntax;
				if (i % 2 == 0) {
					syntax = _generatorFull.GetSyntaxTree(maxTreeDepth);
				} else {
					syntax = _generatorGrow.GetSyntaxTree(maxTreeDepth);
				}
